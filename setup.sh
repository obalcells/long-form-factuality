...
setup_git() {
    ...
    echo "Please enter your Github PAT (press Enter to skip):"
    read -r token
    if [ -n "$token" ]; then
        git config --global credential.helper store

        echo "Storing GH_TOKEN in config files..."
        echo 'export GH_TOKEN="'$token'"' >> ~/.bashrc
        echo 'export GH_TOKEN="'$token'"' >> ~/.zshrc
    ...
    git config --global user.name "Oscar Balcells Obeso"
    git config --global user.email "balcells.oscar@gmail.com"
}
